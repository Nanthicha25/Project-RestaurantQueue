    ...
    {  
       ...
       BufferedReader CheckR3=new BufferedReader(CheckR2);
        ...
        {String text[]=s.split(",");
         if(text.length>1&&text[1].equals(foods.getId()))   
         {
            datafilestock.updateStock(foods.gettype()+"Stock",foods);
         }
        }
        CheckR3.close(); CheckR2.close();
         break; }
         if(updatetext.length>1&&updatetext[0].equals(foods.getName()))
         {
           throw new InvalidOperationException("Duplicat Name : "+foods.getName()+" in "+updatetext[3]);
         }
         
        if(updatetext.length>1&&updatetext[1].equals(foods.getId())&&!updatetext[0].equals(foods.getName()))
         {
           throw new InvalidOperationException("Can not change namefood");
         }
         if(updatetext.length>1&&updatetext[1].equals(foods.getId())&&!updatetext[3].equals(foods.gettype()))
         {
