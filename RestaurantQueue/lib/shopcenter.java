 ...
 //เมธอดเพิ่มอาหารเข้าตะกร้า
  ...
  if(comment==null||comment.isBlank()||comment.isEmpty())
  {
    comment="No comment";
      ...
      buyfood.deletecart(namefood,comment);
        ...
        public void Decreasecart(String namefood,int quantity,String comment) throws goodsNotFoundException, InvalidOperationException
      { 
        if(comment==null||comment.isBlank()||comment.isEmpty())
       {
       comment="No comment";
        ...
        if(namefood==null||namefood.isEmpty()||namefood.isBlank())
  {
    throw new InvalidOperationException("Invalid namefood");
      ...
      buyfood.decreaseQuantity(namefood, quantity, comment);
