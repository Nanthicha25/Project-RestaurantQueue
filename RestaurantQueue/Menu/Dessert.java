...
public class Dessert implements menu {
   ...
   Datafilemenu updatemenu=new Datafilemenu();
    ...
    public void addfood(food foods) throws InvalidOperationException {
        ...
        for(int i=0;i<dessert.size();i++)
        {    if(dessert.get(i).getName().toLowerCase().equals(foods.getName().toLowerCase())&&!dessert.get(i).getId().equals(foods.getId()))
            {   check=false;
                throw new InvalidOperationException(foods.getName()+" Duplicate name Dessert");
            }
            //ถ้ารหัสอาหารซ้ำจะอัพเดทอาหารใหม่
            if(dessert.get(i).getId().equals(foods.getId())&&dessert.get(i).getName().toLowerCase().equals(foods.getName().toLowerCase()))
            { dessert.set(i, new food(dessert.get(i).gettype(), dessert.get(i).getId(), dessert.get(i).getName(),foods.getPrice()));
            check=false;  }
    ...
    // { 
    //    return updatemenu.findbyId(Id,"Dessert");
     //   }
