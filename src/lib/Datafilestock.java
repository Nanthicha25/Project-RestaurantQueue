...
double price=0;
    ...
    if(R2 != null) R2.close();
        ...
        {price=Double.parseDouble(text[2]); imagestock=text[4];
            check=false;}}
        R3.close();
    R2.close();}
   ...
   datafilestock.add(namefood+","+id+","+price+","+namestock+","+imagestock);
        ...
        try {
            ...
            for (int i = 0; i < datafilestock.size(); i++) {
                ...
                if (text.length > 1 && text[1].equals(foods.getId())) {
                    ...
                    imagestock=text[4];
                    datafilestock.set(i, foods.getName() + "," + foods.getId() + "," + foods.getPrice() + "," + foods.gettype()+ "," +imagestock);
     ...
     {List<String>Showallstock=new ArrayList<>();
      ...
      datafilestock.clear();
        try {R1=new File(namestock);
          ...
          for(int i=0;i<Showallstock.size();i++)
          {String text[]=Showallstock.get(i).split(",");
          if(text.length>3)
          { datafilestock.add(text[0]+","+text[1]+","+text[2]+","+text[3]);}
          }
     } catch (Exception e) {
        System.out.println(e.getMessage());
     }finally{
...
try {   String stock[]={"MaincourseStock","DessertStock","DrinksStock"};
        ...
        {  R1=new File(filestock);
          ...
          if(text[0].equals(namefood))
            {datafilestock.add(text[0]+","+text[1]+","+text[2]+","+text[3]); break;}
          }
        if (R3 != null) R3.close();
        if (R2 != null) R2.close();
        if(!datafilestock.isEmpty())
